...
function Wit(opts) {
  ...
  this.converse = (sessionId, message, context, res) => {
  ...
  const continueRunActions = (sessionId, message, prevContext, res, appUser, i) => {
    return (json) => {
      if (i < 0) {
      ...
      if (json.type === 'msg') {
        ...
        return actions.send(request, response, res, appUser).then((ctx) => {
          if (ctx) {
            throw new Error('Cannot update context after \'send\' action');
          }
          return this.converse(sessionId, null, prevContext).then(
            continueRunActions(sessionId, message, prevContext, res, appUser, i - 1)
          );
        });
        ...
        return actions[action](request).then((ctx) => {
          ...
          return this.converse(sessionId, null, nextContext).then(
            continueRunActions(sessionId, message, nextContext, res, appUser, i - 1)
          );
        });
      } else {
  ...
  this.runActions = (sessionId, message, context, res, appUser, maxSteps) => {
    ...
    return this.converse(sessionId, message, context).then(
      continueRunActions(sessionId, message, context, res, appUser, steps)
    );
  };
...
const throwIfActionMissing = (actions, action) => {
  if (!actions[action]) {
    throw new Error('No \'' + actions[action] + '\' action found.');
